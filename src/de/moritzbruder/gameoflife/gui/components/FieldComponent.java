...
import de.moritzbruder.gameoflife.gui.helper.JClickableComponent;
import de.moritzbruder.gameoflife.gui.interaction.FieldMenu;
...
public class FieldComponent extends JClickableComponent {
    ...
    public void paintComponent(Graphics g) {
        ...
        for (int i = 0; i <= field.getWidth(); i++) {
            ...
            g.drawLine(x + drw.getHoriOffset(), drw.getVertOffset(), x + drw.getHoriOffset(), this.getHeight() - drw.getVertOffset());
        }

        //Draw horizontal grid-lines
        for (int i = 0; i <= field.getHeight(); i++) {
            int y = (i * (drw.getDrawHeight() - 1)) / field.getHeight();
            g.drawLine(drw.getHoriOffset(), y + drw.getVertOffset(), this.getWidth() - drw.getHoriOffset(), y + drw.getVertOffset());
        }
     ...
     * @param drawOffsets The draw offsets that apply to the current config
     * @throws IndexOutsideFieldException thrown when the requested pixelCoords are outside the actual grid, but in the border of the component
